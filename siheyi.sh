...
echo -n "please enter 应用部署目录 物种名 mysql的root密码 mysql库 mongodb的admin库用户 mongodb的admin库密码 redis密码  服务器IP ->"
read project_dir species_name mysqlroot_password mysql_db mongoadmin_user mongoadmin_password redis_password
...
sleep 3
# 安装nginx和tomcat
species_dir="${project_dir}/gooalgene/dbs/${species_name}"
...
mkdir -p ${project_dir}/gooalgene/dbs/nginx/conf
cat <<EOF>> ${project_dir}/gooalgene/dbs/nginx/docker-compose.yml
nginx_${species_name}:
   restart: always
   image: nginx
...
tomcat_${species_name}:
  ...
  #    - ./soybean/soybean-dna:/usr/local/tomcat/webapps/soybean-dna
       - ./dna:/usr/local/tomcat/webapps/soybean-dna
       - ./mrna:/usr/local/tomcat/webapps/soybean-mrna
       - ./qtl:/usr/local/tomcat/webapps/soybean-qtl
       - ./iqgs:/usr/local/tomcat/webapps/soybean-iqgs
       - /etc/localtime:/etc/localtime
       - /etc/timezone:/etc/timezone
   expose:
       - 8080
EOF
cat <<EOF>>  ${project_dir}/gooalgene/dbs/nginx/conf/nginx.conf
user  nginx;  
worker_processes  1;
...
EOF
cat <<EOF>>  ${project_dir}/gooalgene/dbs/nginx/conf/nginx_reverse.conf
...
server {
	...
	location /soybean-dna/webSocket{
        proxy_pass http://tomcat_${species_name}/soybean-dna/webSocket;
    ...
    location /soybean-mrna/webSocket{
        proxy_pass http://tomcat_${species_name}/soybean-mrna/webSocket;
        proxy_connect_timeout 4s;
        proxy_read_timeout 10800s;
        proxy_send_timeout 12s;
        proxy_set_header Upgrade $http_upgrade;
    ...
    location /soybean-qtl/webSocket{
        proxy_pass http://tomcat_${species_name}/soybean-qtl/webSocket;
        proxy_connect_timeout 4s;
        proxy_read_timeout 10800s;
        proxy_send_timeout 12s;
        proxy_set_header Upgrade $http_upgrade;
    ...
    location /soybean-iqgs/webSocket{
        proxy_pass http://tomcat_${species_name}/soybean-iqgs/webSocket;
        proxy_connect_timeout 4s;
        proxy_read_timeout 10800s;
        proxy_send_timeout 12s;
        proxy_set_header Upgrade $http_upgrade;
...
EOF
cat <<EOF>> ${project_dir}/gooalgene/dbs/nginx/conf/server.xml
...
EOF
cd ${project_dir}/gooalgene/dbs/${species_name}
docker-compose up -d
# 安装mysql
